...
const Home = () => {
  ...
  return (
    <div className="bg-background">
      {/* ── Hero Section ── */}
      <section className="bg-gradient-to-br from-primary via-primaryDark to-[#0a0f1a] text-white">
        <div className="max-w-[1400px] mx-auto px-4 py-20 md:py-28 flex flex-col md:flex-row items-center gap-10">
          {/* Left Text */}
          <div className="flex-1">
            <span className="inline-block bg-secondary/20 text-secondary text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full mb-5">
              🔥 New Season Deals
            </span>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <Link
                ...
                className="border border-white/30 text-white px-8 py-3.5 text-base rounded-md hover:bg-white/10 transition-colors font-medium"
              >
                Browse Electronics
              </Link>
            </div>
          ...
          <div className="flex-1 flex justify-center">
            <div className="relative w-72 h-72 md:w-96 md:h-96">
              ...
              <div className="relative grid grid-cols-2 gap-4 p-6">
                {['📱', '💻', '🎮', '🎧'].map((emoji, i) => (
                  <div key={i} className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 text-4xl flex items-center justify-center border border-white/20 hover:bg-white/20 transition-colors cursor-default">
                    {emoji}
                  </div>
                ))}
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {categories.map((cat) => (
            <button
              ...
              className="group bg-surface border border-border rounded-xl p-5 text-left hover:border-secondary hover:shadow-lg transition-all duration-300"
            >
      ...
      <section className="bg-gradient-to-r from-secondary to-secondaryHover text-white">
        <div className="max-w-[1400px] mx-auto px-4 py-16 text-center">
          ...
          <Link
            to="/store"
            className="bg-white text-secondary font-black px-10 py-4 rounded-md inline-flex items-center gap-2 hover:bg-gray-50 transition-colors text-sm uppercase tracking-wider shadow-xl"
          >
