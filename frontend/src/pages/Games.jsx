...
const Games = () => {
  ...
  const playGame = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        toast.success(`Spin Result: ${res.result}`);
      }, 2600);
    } catch (err) {
  ...
  if (loading) {
    return <div className="p-12 text-center text-textMuted text-xs uppercase tracking-widest">Game status…</div>;
  }
  ...
  return (
    <div className="max-w-[1000px] mx-auto px-4 py-8">
      <div className="card p-8">
        ...
        <p className="text-textMuted mb-6">One spin every 24 hours. Spin now and claim coupon if rewarded.</p>
        ...
        <div className="mb-7 flex flex-col items-center">
          ...
          <div className="mt-3 text-xs uppercase tracking-widest text-textMuted">Wheel: 10%, 15%, 20%, Better Luck, No Reward</div>
        </div>

        <div className="border border-border p-4 mb-6 bg-backgroundElevated">
          <div className="text-xs uppercase tracking-wider text-textMuted mb-1">Status</div>
          <div className="text-lg font-mono font-bold text-secondary">{statusText}</div>
          {status?.lastResult && (
            <div className="mt-2 text-sm text-textMain">
              Last result: <span className="font-bold uppercase">{spinResult || status.lastResult}</span>
              {status.rewardGiven ? ' • Reward available' : ' • No reward this round'}
            </div>
          )}
        </div>
        ...
        {lastCoupon && (
          <div className="mt-6 border border-secondary/50 p-4 bg-secondary/10">
            <div className="text-xs uppercase tracking-wider text-textMuted mb-1">Coupon Awarded</div>
            <div className="font-mono font-bold text-secondary">{lastCoupon.code}</div>
            <div className="text-sm text-textMain mt-1">Discount: {lastCoupon.discount}%</div>
          </div>
        )}
      </div>
    </div>
